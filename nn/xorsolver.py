import numpy as np
import nn.base.layer.layer as layer
...
w_in_to_hide = layer.init(2,2)
w_hide_to_out = layer.init(2,1)
...
for ite in range(maximum_iter):
  ...
  X = in_data[:,0:2]
  Y = in_data[:,2]
  hide, cache_hide = layer.forward(X,w_in_to_hide,"sigmoid")
  output, cache_out = layer.forward(hide,w_hide_to_out,"sigmoid")
  dy = output - Y
  loss, dhide,dw_hide_to_out = layer.backward(dy,cache_out)
  _, _, dw_in_to_hide = layer.backward(dhide,cache_hide)
...
for i in range(4):
  ...
  X = in_data[:,0:2]
  Y = in_data[:,2]
  hide, _ = layer.forward(X,w_in_to_hide,"sigmoid")
  output, _ = layer.forward(hide,w_hide_to_out,"sigmoid")
